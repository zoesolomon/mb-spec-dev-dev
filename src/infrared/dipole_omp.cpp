...
inline double distance3(size_t imcon, const double* cell, const double* rcell, const double* r1, const double* r2)
{
...
inline void image(const size_t imcon, const double* cell, const double* rcell, const double* r1, const double* r2, double* crd_out)
{
    ...
    if (imcon == 1 || imcon == 2) {
       ...
       const double ssz = cell[2]*dx + rcell[5]*dy + rcell[8]*dz;
...
inline void image_dimer(const size_t imcon, const double* cell, const double* rcell, const double* r1, const double* r2, double* dimer)
{
...
int main(int argc, char** argv)
{
    ...
    if (argc < 3) {
        ...
        std::cerr << "usage: " << argv[0] << "[nthreads] <file_name> <mb-spec.json>" << std::endl;
    ...
    std::cerr << "Using "  << nthreads << " OpenMP threads" << std::endl;
    ...
    if (imcon == 0) {
        ...
        std::cerr << "imcon = 1 (cubic)" << std::endl;

    } else if (imcon == 2) {

        std::cerr << "imcon = 2 (orthorhombic)"  << std::endl;

    } else if (imcon == 3) {

        std::cerr << "imcon = 3 (monoclinic)" << std::endl;
    }
    ...
    while(!ifs_cmd.eof()) {
        ...
        while((int)frames.size() < 32 && !ifs_cmd.eof()) {
